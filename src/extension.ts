...
export function activate(context: vscode.ExtensionContext) {
	...
	const disposable = vscode.commands.registerCommand('vscode-activity-journal.helloWorld', () => {
		// The code you place here will be executed every time your command is executed
		// Display a message box to the user
		vscode.window.showInformationMessage('Hello World from vscode-activity-journal!');
	});

	context.subscriptions.push(disposable);

	// Example: Track opened documents
	...
	// Example: Track saved documents
	const saveListener = vscode.workspace.onDidSaveTextDocument((document) => {
		console.log(`Saved: ${document.fileName}`);
	});
	context.subscriptions.push(saveListener);

	// Example: Track closed documents
